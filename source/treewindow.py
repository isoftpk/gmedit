...
class TreeWindow(QtGui.QMainWindow):
    ...
    def __init__(self, model, allActions, parent=None):
        ...
        self.treeView.selectionModel().selectionChanged.connect(self.selectChanged)
        self.treeView.selectionModel().selectionChanged.connect(self.updateRightViews)
        self.treeFilterView = treeview.TreeFilterView(model, self.treeView.selectionModel(),self.allActions)
        self.treeFilterView.shortcutEntered.connect(self.execShortcut)
        self.treeStack.addWidget(self.treeFilterView)
        self.treeView.selectionModel().selectionChanged.connect(self.treeFilterView.updateFromSelectionModel)
        ...
        self.rightTabs.addTab(self.editorSplitter, _('Edit'))
        parentEditView = dataeditview.DataEditView(self.treeView.selectionModel(),self.allActions, False)
        parentEditView.nodeModified.connect(self.nodeModified)
        parentEditView.inLinkSelectMode.connect(self.treeView.toggleNoMouseSelectMode)
        parentEditView.inLinkSelectMode.connect(self.treeFilterView.toggleNoMouseSelectMode)
        parentEditView.focusOtherView.connect(self.focusNextView)
        parentEditView.shortcutEntered.connect(self.execShortcut)
        self.treeView.skippedMouseSelect.connect(parentEditView.internalLinkSelected)
        self.treeFilterView.skippedMouseSelect.connect(parentEditView.internalLinkSelected)
        self.editorSplitter.addWidget(parentEditView)
        childEditView = dataeditview.DataEditView(self.treeView.selectionModel(),self.allActions, True)
        childEditView.nodeModified.connect(self.nodeModified)
        childEditView.inLinkSelectMode.connect(self.treeView.toggleNoMouseSelectMode)
        childEditView.inLinkSelectMode.connect(self.treeFilterView.toggleNoMouseSelectMode)
        childEditView.focusOtherView.connect(self.focusNextView)
        childEditView.shortcutEntered.connect(self.execShortcut)
        self.treeView.skippedMouseSelect.connect(childEditView.internalLinkSelected)
        self.treeFilterView.skippedMouseSelect.connect(childEditView.internalLinkSelected)
        self.editorSplitter.addWidget(childEditView)

        self.outputSplitter = QtGui.QSplitter(QtCore.Qt.Vertical)
        self.rightTabs.addTab(self.outputSplitter, _('Output'))
        parentOutputView = outputview.OutputView(self.treeView.selectionModel(), False)
        parentOutputView.highlighted[str].connect(self.statusBar().showMessage)
        self.outputSplitter.addWidget(parentOutputView)
        childOutputView = outputview.OutputView(self.treeView.selectionModel(),True)
        ...
        self.rightTabs.addTab(self.titleSplitter, _('Title'))
        parentTitleView = titlelistview.TitleListView(self.treeView.selectionModel(), False)
        ...
        childTitleView = titlelistview.TitleListView(self.treeView.selectionModel(), True)
        ...
        self.rightViewList = [parentEditView, childEditView,
                              parentOutputView, childOutputView,
    ...
    def setCaption(self, filePath=''):
        ...
        """
        if filePath:
            caption = '{0} [{1}] - GmEdit'.format(os.path.basename(filePath),
